...
void InterfaceForHistogramPeople() {
    ...
    if (!change.empty()) {
        ...
        case 'Y': {
            ...
            int variant = 0;
            std::cout << "Выберите гистограмму: 1 - Год рождения, 2 - Рост, 3 - Вес\n";
            std::cin >> variant;

            switch (variant) {
            case 1:
                    PrintHistogram<Person, int>(mySet, getBirthYear, "<Год Рождения>");
                break;
            case 2:
                PrintHistogram<Person, int>(mySet, getHeight, "<Рост>");
                break;
            case 3:
                PrintHistogram<Person, int>(mySet, getWeight, "<Вес>");
                break;
            default:
